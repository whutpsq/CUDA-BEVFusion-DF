...
def encode_detection_message(
    ...
    if boxes is None or scores is None or labels is None:
        return json.dumps(_detection_payload([], timestamp_us, outputs))
    ...
    return json.dumps(_detection_payload(objects, timestamp_us, outputs), separators=(",", ":"))
...
def _detection_payload(
    ...
    if timestamp_us is not None:
        payload["timestamp_us"] = int(timestamp_us)
    if outputs is not None and outputs.get("map_probs") is not None:
        payload["map"] = _encode_map_payload(
            outputs["map_probs"],
            outputs.get("map_classes"),
            float(outputs.get("map_score_threshold", 0.5)),
        )
    return payload


def _encode_map_payload(map_probs: Any, classes: Any, threshold: float) -> Dict[str, Any]:
    probs = _to_numpy(map_probs).astype(np.float32, copy=False)
    if probs.ndim == 4 and probs.shape[0] == 1:
        probs = probs[0]
    if probs.ndim != 3:
        raise ValueError(f"Expected map_probs shape [C,H,W] or [1,C,H,W], got {probs.shape}")
    ...
    return {
        ...
        "data": base64.b64encode(np.ascontiguousarray(mask).tobytes()).decode("ascii"),
    }
