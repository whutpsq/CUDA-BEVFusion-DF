...
class RsclBagFrameRunner:
        ...
        if not decode_only and not decode_images_only:
            ...
            if not cfg.calibration_file and not cfg.allow_identity_calibration:
                raise RuntimeError(
                    ...
                    "Set calibration_file in the adapter config, or use --decode-only/--decode-images-only first."
                )
    ...
    def _handle_frame(self, frame: SyncedFrame) -> None:
        ...
        if self.decode_only:
            ...
            record = json.loads(encode_detection_message(outputs[0], self.cfg.score_threshold, frame.timestamp_us))
